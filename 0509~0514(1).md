...
router를 CDN방식으로 넣을 경우, vue src보다 아래에 적어줘야한다.
	<script src="https://cdn.jsdelivr.net/npm/vue/dist/vue.js"></script>
    	<script src="https://unpkg.com/vue-router/dist/bue-router.js"></script>


38강 (라우터 정리 및 학습 방향 안내)
‘네비게이션 가드’ 블로그 글 링크:
...
8. HTTP 통신 라이브러리 - axios

39강 (HTTP 라이브러리와 Ajax 그리고 Vue Resource)
axios(액시오스) : 뷰에서 권고하는 HTTP 통신 라이브러리.
Ajax : 데이터 받아옴.(Asynchronous JavaScript and XML, 에이잭스).
...
40강 (axios 소개 및 오픈 소스를 사용하기 전에 알아야 할 것들)
Axios 깃헙 주소 :
...
41강 (axios 실습 및 this 설명)
...
42강 (웹 서비스에서의 클라이언트와 서버와의 HTTP 통신 구조)
...
43강 (크롬 개발자 도구 네트워크 패널 보는 방법)
개발자모드의 Network패널.
...
47강 ([실습 풀이] 클래스 바인딩, v-if, v-show)
v-if는 보이지 않게 되었을 때, DOM에서 그 존재 자체를 없애준다.
v-show는 보이지 않게 되었을 때, 유관상으로만 화면에서 없어지고 DOM에 정보는 남아있다.


48강 (모르는 문법이 나왔을 때 공식 문서를 보고 해결하는 방법)
...
51강 (watch 속성 vs computed 속성)
...
54강 (Vue CLI 도구 설치할 때 문제점 해결 방법)
...
55강 (VCLI 2.x와 3.x의 차이점 / 프로젝트 생성 및 서버 실행)
[Vue CLI 2.x]
...
56강 (CLI로 생성한 프로젝트 폴더 구조 확인 및 main.js 파일 설명)
npm, webpack 부분을 따로 살펴보면 좋음.

npm run serve라는 명령어는 vue-cli-service serve를 더 짧고 편하게 적은 것.

public 폴더의 index.htm파일에 적힌 <!-- built files will be auto injected -->  는  빌드된 파일들이 자동으로 연결될 것이라는 의미이다.
즉, index.html로 src폴더 속의 모든 파일들이 연결되는 것.( 최소한의 파일로 변환하여 연결됨)

57강 (싱글 파일 컴포넌트 소개 및 여태까지 배운 내용 적용하는 방법)
- components의 .vue파일 생성시, 싱글파일 컴포넌트 코드 단축키 : def + enter키.


58강 (App.vue와 HelloWorld.vue 설명)
-
...
59강 (싱글 파일 컴포넌트에 배운 내용 적용하여 개발 시작하기)
<template>는 하나의 엘리먼트만 있어야한다.(하나의 태그로 나머지 태그들이 감싸져 있어야함.)



60강 (싱글 파일 컴포넌트 체계에서 컴포넌트 등록하기)
components의 파일들을 추가할 때에는 파스칼 형식(단어단어마다 시작을 대문자로 작성)으로 작성해야함. 예) AppHeader.vue
또한, 최소한 두 단어 이상으로 지어줘야한다. 예를 들어, main이라는 컴포넌트를 만들었을 때, App.uve에서 <main></main>이라고 치면 기존의 main.js파일인지, 컴포넌트인 main.vue파일인지 알 수 없기 때문에 두 단어 이상으로 작성하는 것을 권고한다.
...
66강 (axios를 이용한 데이터 전송 및 form 구현)
...
67강 (수업 정리 및 향후 학습 방향 안내)
Reactivity : vue.js가 추구하는 중심 사상이자 핵심 기능이다. 웹개발에 대한 동작들을 라이브러리에서 좀더 편하게 관리해 준다는 것이 reactivity의 핵심.
인스턴스 : vue로 개발을 할 때 필수로 생성해야하는 단위이자, 실제로 인스턴스 안에 어떤 내용들을 추가함으로써 화면을 조작할 수 있다.
컴포넌트 : 화면을 영역을 구분해서 개발할 수 있음. 컴포넌트 개발이 프론트엔드 프레임워크에서 가장 중요한 개발이다. 재사용성 즉, 코드의 반복을 줄이는 것이 핵심.
컴포넌트 통신 :  컴포넌트로 개발할 때 데이터의 흐름을 제어하기 위한, 즉, 데이터의 규칙들을 제한하면서 개발할 수 있다. 제한을 하면서 생기는 이점들은 여러명이 협업해도 데이터의 흐름을 예측할 수 있는 것.
 props : 상위에서 하위로 데이터이동.
 event emit : 하위에서 상위로 데이터 이동.
HTTP 통신 라이브러리 (axios) : axios 깃헙 레퍼지토리에서 더 자세하게 살펴보면 좋습니다.
템플릿 문법 : 화면을 조작하기 위한 vue의 문법들.
데이터 바인딩 : Reactivity와 비슷한 개념. 데이터의 변화에 따라서 화면에 반영해주고 실제로 그 데이터를 화면에 엮어내는 것.
뷰 디렉티브 : 화면을 조작하기 위해서 vue가 추가적으로 제공하는 문법. (v-) ex) v-bind, v-on….
Vue CLI : 프로젝트를 생성할 때 명령어를 이용해서 생성하는 방식.
싱글 파일 컴포넌트 : Vue CLI 아래에서 생성되는 .vue파일. webpack의 기능에 대해 더 공부하면 좋습니다.
